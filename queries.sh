...
# Using winner_id and joining with the teams table to get the team name
echo "$($PSQL "SELECT t.name FROM games g
              JOIN teams t ON g.winner_id = t.team_id
              WHERE g.year = 2018 AND g.round = 'Final'")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
# Getting both winner and opponent teams in the 'Eighth-Final' round
echo "$($PSQL "
SELECT t.name
FROM games g
JOIN teams t ON g.winner_id = t.team_id
...
# Query to get all distinct winning team names
echo "$($PSQL "SELECT DISTINCT t.name FROM games g
              JOIN teams t ON g.winner_id = t.team_id
              ORDER BY t.name")"

echo -e "\nYear and team name of all the champions:"
# Query to get year and winner of the final match
...
# Query to get teams that start with 'Co'
